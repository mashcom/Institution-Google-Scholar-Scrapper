...
class ScholarlyInstitution:
    ...
    def get_institution_link(self):
        for i in trange(10, file=sys.stdout, desc='Getting Parent Link'):
    ...
    def get_institution_affiliates(self, link):
        ...
        while can_progress:
            for affiliate_index in trange(10, file=sys.stdout, desc='Progress Report'):
                page = requests.get(url)
                soup = BeautifulSoup(page.text, 'html.parser')
                ...
                if next_page is not None:
                    ...
                    a_start = i * 10
                    next_url = "https://scholar.google.com/citations?view_op=view_org&hl=en&org=" + organisation_id + "&after_author=" + after_author_param + "&astart=" + a_start.__str__()
                    url = next_url
